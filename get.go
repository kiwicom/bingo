...
func getPackage(ctx context.Context, logger *log.Logger, c installPackageConfig, i int, name string, target bingo.Package) (err error) {
	...
	tmpDir, err := os.MkdirTemp(c.modDir, name+".dir.tmp")
	if err != nil {
		return err
	}
	...
	tmpEmptyModFilePath := filepath.Join(tmpDir, name+"-e.tmp.mod")
	tmpModFilePath := filepath.Join(tmpDir, "go.mod")
	tmpGoFilePath := filepath.Join(tmpDir, "deps.go")
	if i > 0 {
	...
	err = writeDepsGoFile(target.Path(), tmpGoFilePath)
	if err != nil {
		return err
	}
	...
	if err := install(ctx, logger, c.runner, c.modDir, name, c.link, tmpModFile, tmpDir); err != nil {
	...
	if err := os.RemoveAll(tmpDir); err != nil {
		return errors.Wrap(err, "rm tmp dir")
	}
	return nil
}

func writeDepsGoFile(packagePath, filePath string) error {
	f, err := os.OpenFile(filePath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0666)
	if err != nil {
		return err
	}
	_, err = fmt.Fprintf(f, `package main // Auto generated by https://github.com/bwplotka/bingo. DO NOT EDIT
...
func install(ctx context.Context, logger *log.Logger, r *runner.Runner, modDir string, name string, link bool, modFile *bingo.ModFile, tmpDir string) (err error) {
	...
	if _, err := r.With(ctx, modFile.FileName(), tmpDir, nil).ModTidy(); err != nil {
		return errors.Wrap(err, "tidy")
	}
	...
	listArgs = append(listArgs, "-mod=readonly", "-f={{.Name}}", pkg.Path())
